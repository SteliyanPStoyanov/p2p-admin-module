...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <form id="loginAttemptForm" class="form-inline card-body"
                      action="{{ route('admin.login-attempt.list') }}"
                      method="PUT">
                    {{ admin_csrf_field() }}
            ...
            <div id="main-table" class="card">
                <div class="card-body pt-3">
                    <div class="table-responsive">
                        <div id="loginAttempt">
                            @include('admin::login-attempt.list-table')
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        loadSimpleDataGrid('{{ route('admin.login-attempt.refresh') }}', $("#loginAttemptForm"), $("#loginAttempt"), true, 10000);

        $(document).ready(function () {

            $("#maxRows").change(function () {
                let routeRefreshLoginAttempt = '{{ route('admin.login-attempt.refresh')}}';

                $.ajax({
                    type: 'get',
                    url: routeRefreshLoginAttempt,
                    data: $('#loginAttemptForm').serialize(),

                    success: function (data) {
                        $('#loginAttempt').html(data);
                    },
                });
            });
        });
