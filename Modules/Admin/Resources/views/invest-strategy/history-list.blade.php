@foreach($investStrategyHistory as $investStrategy)
    ...
    <tr>
        ...
        <td class="text-center">{{ formatDate($investStrategy->created_at , 'd.m.Y H:i:s')}}</td>
        <td class="text-center">{{ formatDate($investStrategy->archived_at , 'd.m.Y H:i:s')}}</td>
...
<tr id="pagination-nav">
    <td colspan="17">
        {{ $investStrategyHistory->onEachSide(1)->links() }}
    </td>
</tr>
