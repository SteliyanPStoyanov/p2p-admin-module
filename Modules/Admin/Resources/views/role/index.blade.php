...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <form id="roleForm" class="form-inline card-body" action="{{ route('admin.roles.list') }}"
                      method="PUT">
                    {{ admin_csrf_field() }}
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <div id="btns-panel">
                        <x-btn-create url="{{ route('admin.roles.create') }}" name="{{ __('common.Create') }}"/>
                    </div>

                    <div id="main-table">
                        <div class="table-responsive">
                            <div id="role-table">
                                @include('admin::role.list-table')
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        loadSimpleDataGrid('{{ route('admin.roles.refresh') }}', $("#roleForm"), $("#role-table"));
    </script>
@endpush
